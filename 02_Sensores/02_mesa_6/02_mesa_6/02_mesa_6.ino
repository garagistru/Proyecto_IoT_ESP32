...
// ├──────────────────┼──────────────────────────────────┼─────────────────────┤
// │ setname:NAME     │ Установить имя станка            │ —                   │
// │                  │ Пример: setname:mesa_7           │                     │
// │                  │ Сохраняется в NVS (Preferences)  │                     │
// ├──────────────────┼──────────────────────────────────┼─────────────────────┤
// │ info             │ Показать состояние устройства:   │ —                   │
// │                  │  - имя, MAC, boot_id             │                     │
// │                  │  - № смены, счётчик нажатий      │                     │
// │                  │  - статус (ACTIVO/PASIVO)        │                     │
// │                  │  - очередь (count/MAX, head)     │                     │
// │                  │  - Wi-Fi, время, uptime          │                     │
// ├──────────────────┼──────────────────────────────────┼─────────────────────┤
// │ send             │ Отправить ОДИН пакет из очереди  │ —                   │
// │                  │ немедленно (не ждать 30 сек)     │                     │
// │                  │ Требует активного Wi-Fi          │                     │
// ├──────────────────┼──────────────────────────────────┼─────────────────────┤
// │ clear            │ Очистить очередь (все q_0..q_19) │ Нужно ввести 'yes'  │
// │                  │ Сбрасывает queueHead и queueCount│ в течение 5 секунд  │
// │                  │ НЕ трогает номер смены           │ Иначе — отмена      │
// ├──────────────────┼──────────────────────────────────┼─────────────────────┤
// │ resetshift       │ Сбросить номер смены на #1       │ Нужно ввести 'yes'  │
// │                  │ и обнулить pressCount            │ в течение 5 секунд  │
// │                  │ Сохраняется в NVS                │ Иначе — отмена      │
// │                  │ НЕ трогает очередь               │                     │
// ├──────────────────┼──────────────────────────────────┼─────────────────────┤
// │ sync             │ Пересинхронизировать время       │ —                   │
// │                  │ у головного (GET /time)          │                     │
// │                  │ Сбрасывает isSynced, вызывает    │                     │
// │                  │ syncFromGateway()                │                     │
// ├──────────────────┼──────────────────────────────────┼─────────────────────┤
// │ reboot           │ Перезагрузить ESP32              │ —                   │
// │                  │ (ESP.restart(), задержка 500 мс) │                     │
// ├──────────────────┼──────────────────────────────────┼─────────────────────┤
// │ help             │ Показать список всех команд      │ —                   │
// └──────────────────┴──────────────────────────────────┴─────────────────────┘
//
// ПРИМЕЧАНИЯ:
//  - clear и resetshift требуют ввода 'yes' в течение 5 секунд
//    (защита от случайного нажатия). Любой другой ввод = отмена.
//  - resetshift НЕ очищает очередь. Для очистки — отдельная команда clear.
//  - clear НЕ сбрасывает номер смены. Для сброса — отдельная resetshift.
//  - setname: сохраняется в namespace "mesa6_data", ключ "dev_name".
//  - После reboot все данные (смена, очередь, имя) восстанавливаются из NVS.
// ============================================================
